...
void B1SteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  if (step->GetTrack()->GetTrackID() == 1)
  //若当前的step是初级粒子对应的step
  {
    if (step->GetPostStepPoint()->GetPosition().z() == -4.1)
    //当前step的PostStepPoint的z分量是-2.5mm，即z坐标达到了电离室的出口位置
    {
      G4double TotalEnergyPostStep = step->GetPostStepPoint()->GetKineticEnergy();
      G4double TotalEnergyStep = 240 - TotalEnergyPostStep;
      fEventAction->AddTotalEnergy(TotalEnergyStep);
    }
  }
}
